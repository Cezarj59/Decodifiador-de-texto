...
function verificaCaracteres(texto) {
    // se for detectado algum caractere em letras Maiuscula ou igual aos da const caracteresInvalidos, retorna true, caso contrario false.
    ...
    // console.log(caracteresInvalidosArray);
    ...
    while (index < textoArray.length) {
        ...
        if (/[A-Z]/.test(textoArray) || textoArray[index] == caracteresInvalidosArray[contador]) {
            // console.log('Por favor digite sem letras maiusculas, acentos e numeros');
...
function codifica(texto) {
    //  faz a codificação do texto

    let textoDigitado = texto;

    // colocando cada caractere digitado em um array.
    let distribuidoNoArray = [...textoDigitado];
    // console.log(distribuidoNoArray);
    ...
    var textoCodificado = distribuidoNoArray.join('');
...
function descriptografar() {
    ...
    if (document.getElementById('digiteTexto').value == "") {
        ...
        limpaAreaTexto()
        for (var indice = 0; indice < arrayDePalavras.length; indice++) {
...
function limpaAreaTexto() {
    const limpaAreaTexto = document.getElementById('areaDecodificacao').innerHTML = "";
    const limpaButton = document.getElementById('secondArea').innerHTML = "";
}
function limpaAreaDigitada() {
    const limpaAreaDigitada = document.getElementById('digiteTexto').value = "";
}
